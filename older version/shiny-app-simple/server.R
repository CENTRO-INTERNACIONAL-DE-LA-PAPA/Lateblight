...
server <- function(input, output) {
  ...
  observeEvent(input$mymap1a_click, {
    ...
    leafletProxy('mymap1a') %>%
      addMarkers(lng = clng, lat = clat, popup = paste(round(clat, 4), round(clng, 4), sep = ", "),
                 icon = list(
                   iconUrl = 'https://icons.iconarchive.com/icons/icons8/ios7/256/Maps-Location-icon.png',
                   iconSize = c(30, 30))) 
  ...
  output$var_names1 <- renderUI({
    ...
    names_var_per <- read.csv("potato-res-per.csv", header = T, sep = ";", stringsAsFactors = F,
                             encoding = "UTF-8")
    ...
    names_var_ecu <- read.csv("potato-res-ecu.csv", header = T, sep = ";", stringsAsFactors = F,
                              encoding = "UTF-8")
    ...
    selectInput("res", "Select the potato variety:", 
                choices = my_list_per , selectize = FALSE)
    } else{
      selectInput("res", "Select the potato variety:", 
                  choices = my_list_ecu , selectize = FALSE) 
    }
  ...
  climDataAPI_input_model <- function(day0, dayn, lat, long, hrlimite, calculate_hhr) {
    ...
    if((dayinitial < sysdate) == TRUE && (dayfinal < sysdate) == TRUE) {
      ...
      if(difftime_day <= 30) {
        ...
        PreDataAPI <- paste(URL_base, API_Method_history, API_key, API_q_coordinates, API_date, sep = '')
        ...
        hr_hours2 <- aggregate(hr ~ date_daily, climData, 
                               FUN = function(RH) {
                                 return(length(RH[RH>as.numeric(hrlimite)]))
                               })
        
        input_runsimcast <- data.frame(hr_hours2, tmean2[,2], pp2[,2])
      ...
      # input-runsimcast: date, hr>90, temp avg, rain
        ...
        seq_API_date <- unique( c(seq(from = dayinitial, to = dayfinal, by =30), dayfinal) )
        ...
        for (i in 1:length(seq_API_date)) {
          ...
          API_date <- paste('&dt=', dayinitial2, '&end_dt=', dayfinal2, sep = '')
      ...
      if(difftime_day_h <= 30) {
        ...
        API_date <- paste('&dt=', dayinitial_h, '&end_dt=', dayfinal_h, sep = '')
        ...
        input_runsimcast_h <- data.frame(hr_hours2, tmean2[,2], pp2[,2])
        
        names(input_runsimcast_h) <- c("date", "hr90_hour", "tavg_C", "rain_mm")
        ...
        seq_API_date <- unique( c(seq(from = dayinitial_h, to = dayfinal_h, by =30), dayfinal_h) )
        ...
        input_runsimcast_h <- do.call(rbind, list_input_runsimcast_h)
      ...
      if(difftime_day_f <= 30) {
        ...
        API_date <- paste('&days=', (as.Date(dayfinal_f) - as.Date(dayinitial_f)+1), sep = '')
        
        PreDataAPI <- paste(URL_base, API_Method_forecast, API_key, API_q_coordinates, API_date, sep = '')
        print(PreDataAPI)
        ...
        input_runsimcast_f <- data.frame(hr_hours2, tmean2[,2], pp2[,2])
        
        names(input_runsimcast_f) <- c("date", "hr90_hour", "tavg_C", "rain_mm")
      ...
      input_runsimcast_h_y_f <- rbind(input_runsimcast_h, input_runsimcast_f)
      ...
      API_date <- paste('&days=', (as.Date(dayfinal) - as.Date(dayinitial)+1), sep = '')
 ...
 ###############################   
  ...
  observeEvent(input$run1, {
    shinyalert("Recomendaciones:", 
               "Usted puede aplicar un fungicida del tipo...", 
               type = "info")
  })
  ...
  output$calendars <- renderSlickR({
    ...
    plot_function <- function(df_output_runsimcast){
      ...
      data_final <- data.frame(as.POSIXct(df_output_runsimcast[,1], format = '%Y-%m-%d'), data_binaria)
